...
@section('content')
   <article class="content responsive-tables-page">
        <div class="title-block" style="display: flex; flex: 1">
            <h1 class="title" style="flex: 1"> Users </h1>
                                                ...
                                                @foreach($users as $user)
                                                    <tr class="odd gradeX">
                                                        ...
                                                        {{-- <td class="center" style="display: flex;"> 
                                                            <a href="{{ route('status.show', $user->id) }}" class="btn btn-pill-left btn-secondary">
                                                                <i class="fa fa-eye"></i>
                                                            </a>
                                                            <a href="{{ route('status.edit', $user->id) }}" class="btn btn-secondary">
                                                                <i class="fa fa-edit"></i>
                                                            </a>
                                                            <form action="{{ route('status.destroy', $user->id) }}" method="POST">
                                                                {{ method_field('DELETE') }}
                                                                {{ csrf_field() }}
